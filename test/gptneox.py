...
##############################################################################33
# int8 section
class GPTNeoXAttentionInt8(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.num_attention_heads = config.num_attention_heads
        self.hidden_size = config.hidden_size
    ...
    def forward(
        ...
        #hidden_states,
        attention_mask,
        q_quant, k_quant, qk_quant, v_quant, requant,
        head_mask=None,
        layer_past=None,
        use_cache=False,
        output_attentions=False,
        ...
        # quant
        query = (query * q_quant).round().clamp(-128, 127).to(torch.float32)
        key = (key * k_quant).round().clamp(-128, 127).to(torch.float32)
        value = (value * v_quant).round().clamp(-128, 127).to(torch.float32)
        # Compute attention
        attn_output, attn_weights = self._attn(query, key, value, attention_mask, head_mask, q_quant, k_quant, qk_quant, v_quant)
        attn_output = (attn_output * requant).round().clamp(-128, 127).to(torch.float32)
    ...
    def _attn(self, query, key, value, attention_mask, head_mask, q_quant, k_quant, qk_quant, v_quant):
        # q, k, v: [bs, num_attention_heads, seq_len, attn_head_size]
        # compute causal mask from causal mask buffer
        batch_size, num_attention_heads, query_length, attn_head_size = query.size()
        key_length = key.size(-2)
        ...
        attn_scores = torch.baddbmm(
            ...
            key.transpose(1, 2),
            beta=1.0,
            alpha=(torch.tensor(1.0, dtype=self.norm_factor.dtype, device=self.norm_factor.device) / self.norm_factor * (1 / q_quant) * (1 / k_quant)),
        )
        attn_scores = attn_scores.view(batch_size, num_attention_heads, query_length, key_length)
...
def gen_ref_gpt_int8(qkvs, beam_idxs, attn_masks, q_quant, k_quant, qk_quant, v_quant, requant):
    class FakeConfig:
        def __init__(self):
            self.num_attention_heads = HEAD_NUM
            self.hidden_size = HIDDEN_SIZE
            ...
            self.rotary_emb_base = ROTARY_EMB_BASE
    config = FakeConfig()
    net = GPTNeoXAttentionInt8(config)
    seq_offset = 0
    results = []
    present = None
    for (i, qkv_) in enumerate(qkvs):
        ...
        seq_offset += qkv.shape[1]
        results.append(attn_output.clone().to(dtype=torch.int8).detach().numpy())
    return results

# test matmul with s8 input and whole output type is s8
def gen_ov_gpt_i8(qkvs, beam_idxs, attn_masks, q_quant=0.0, k_quant=0.0, qk_quant=0.0, v_quant=0.0, requant=0.0):
    def make_model():
        # [batch, tokens, 32*80*3]
        qkv = opset.parameter([-1, -1, HEAD_NUM * SIZE_PER_HEAD * 3], Type.f32, name='qkv')
        past_keys_num = opset.parameter([1,], Type.i64, name='past_keys_num')
...
def test_gpt_i8():
    ...
    q_quant = 127.0 / high
    q = [
            np.random.random(size=[2, 900, HEAD_NUM * SIZE_PER_HEAD]).astype(np.float32)*range_+low,
            np.random.random(size=[2, 1, HEAD_NUM * SIZE_PER_HEAD]).astype(np.float32)*range_+low,
            np.random.random(size=[2, 1, HEAD_NUM * SIZE_PER_HEAD]).astype(np.float32)*range_+low,
            np.random.random(size=[2, 1, HEAD_NUM * SIZE_PER_HEAD]).astype(np.float32)*range_+low,
        ]
    low = -2
    ...
    for i in range(len(q)):
        qkvs.append(np.concatenate([q[i], k[i], v[i]], axis=2))
    beam_idxs = [[-1, -1],
                 [1, 0],
                 [1, 1],
                 [0, 0]]
    ...
    ref_results = gen_ref_gpt_int8(qkvs, beam_idxs, attn_masks, q_quant, k_quant, qk_quant, v_quant, requant)
    ov_results = gen_ov_gpt_i8(qkvs, beam_idxs, attn_masks, q_quant, k_quant, qk_quant, v_quant, requant)
    for (i, ref) in enumerate(ref_results):
